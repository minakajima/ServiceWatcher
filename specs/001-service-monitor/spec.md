...
- Q: 言語切り替えのUI配置 → A: B (設定画面にドロップダウン)
- Q: 言語切り替えの即時反映 → A: A (即時反映、アプリ再起動不要)
- Q: デフォルト言語の決定方法 → A: A (OSの言語設定を検出)
- Q: 言語設定の永続化方法 → A: A (config.json に保存)
- Q: 翻訳対象の範囲 → A: A (すべてのUI要素)
...
### ユーザーストーリー 4 - 言語切り替え (優先度: P3)

システム管理者として、アプリケーションの表示言語を日本語と英語の間で切り替えられるようにし、多言語環境での利用を可能にしたい。

**この優先度の理由**: 国際化対応により、グローバルな環境での利用が可能になり、ユーザーベースの拡大に貢献します。

**独立テスト**: 設定画面で言語を切り替え、すべてのUI要素が即座に選択した言語で表示されることを確認できます。

**受け入れシナリオ**:

1. **前提** アプリケーションが日本語OSで初回起動、**操作** アプリケーションを起動、**結果** UIがすべて日本語で表示される
2. **前提** アプリケーションが英語OSで初回起動、**操作** アプリケーションを起動、**結果** UIがすべて英語で表示される
3. **前提** アプリケーションが起動中、**操作** 設定画面を開き、言語ドロップダウンで「English」を選択、**結果** すべてのUI要素（メニュー、ボタン、ラベル、メッセージ、通知）が即座に英語表示に変更される（再起動不要）
4. **前提** 言語が英語に設定済み、**操作** アプリケーションを再起動、**結果** 英語表示が維持される
5. **前提** 設定画面が開いている、**操作** 言語を「日本語」に戻す、**結果** すべてのUI要素が即座に日本語表示に戻る
...
- OSの言語設定が日本語・英語以外の場合、どの言語をデフォルトとするか？（フォールバック言語）
- 言語切り替え中にサービス停止通知が発生した場合、通知は新旧どちらの言語で表示されるか？
...
- **FR-013**: システムは、日本語と英語の2言語をサポートする必要がある
- **FR-014**: 初回起動時、システムはOSの言語設定（CultureInfo.CurrentUICulture）を検出し、対応する言語でUIを表示する必要がある
- **FR-015**: ユーザーは、設定画面の言語ドロップダウンから表示言語を選択できる必要がある
- **FR-016**: 言語切り替えは即座に反映され、アプリケーションの再起動は不要とする
- **FR-017**: すべてのUI要素（メニュー、ボタン、ラベル、ダイアログメッセージ、通知テキスト、エラーメッセージ）を翻訳対象とする
- **FR-018**: 選択された言語設定は config.json に保存され、次回起動時に読み込まれる必要がある

### 主要エンティティ *(機能がデータを含む場合は含める)*

- **監視対象サービス**: システムで監視されるWindowsサービスを表す。属性: サービス名（内部名）、表示名、現在の状態、最終チェック時刻、通知有効/無効フラグ
- **設定情報**: アプリケーションの動作設定を表す。属性: 監視間隔（秒）、通知表示時間（秒）、設定ファイルパス、監視対象サービスリスト、UI表示言語（"ja" または "en"）
...
- **SC-008**: 言語切り替えは、設定画面で選択後1秒以内にすべてのUI要素に反映される
...
- サポート言語は日本語と英語のみ（OSの言語設定がこれら以外の場合は英語をデフォルトとする）
